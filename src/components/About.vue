<template>
  <div id="webAbout">
    <p>Welcome to your personal dessert recipe website! Here you can search through thousands of
      dessert recipes, perfect for any craving or occasion. Not satisfied with the first 10 results?
      Click more results to see more! If you do see one you like, star it to save it in your
      favorites. Your favorites will save so that the nexttime you visit they will display in the
      sidebar. Click the X button to remove a recipe from your favorites. To see the full recipe
      from its original source, just click the recipe name! </p>
    <p>You can modify your search to limit your results to just the ones you want. Available search
      modifications are by holiday, required ingredients, and ingredients you need to avoid.</p>
    <p>This site was made for the Quarter 1 project for Galvanize's full stack program by Lizzie
      Szoke, a full stack student. If you have any questions about the site, Lizzie, or the
      Galvanize program, just send her an email at szoke.elizabeth@gmail.com!</p>
  </div>
</template>

<script>
export default {
}
</script>

<style lang="css">
@import url('https://fonts.googleapis.com/css?family=Archivo+Narrow:400,700');

#webAbout {
  color: black;
  padding-top: 30px;
}

#webAbout p {
  text-align: left;
  background:rgba(255,255,255,0.5);
  padding: 10px;
  margin: 10px;
  border-radius: 5px;
  font-size: 20px;
  font-family: 'Archivo Narrow';
}

@media(max-width: 527px) {
  #webAbout p {
    font-size: 1em;
  }
}

@media(max-width: 435px) {
  #webAbout p {
    font-size: 0.9em;
  }
}

@media(max-width: 405px) {
  #webAbout p {
    font-size: 0.85em;
  }
}

@media(max-width: 390px) {
  #webAbout p {
    overflow: scroll;
  }
}
</style>
